    ...
    <div className="animate-fade-in">
      ...
      <div className="flex flex-col gap-4 mb-8 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {company === "tbt" && (
            <button onClick={syncFromSalesforce} disabled={sfSyncing} className="btn-secondary text-sm px-3 py-1.5">
              {sfSyncing ? "Syncing..." : "☁ SF Sync"}
            </button>
          )}
          {company === "aex" && (
            <button onClick={syncFromFlybook} disabled={syncing} className="btn-secondary text-sm px-3 py-1.5">
              {syncing ? "Syncing..." : "↻ Flybook"}
            </button>
          )}
          <button onClick={runAnalysis} disabled={runningAnalysis} className="btn-primary text-sm px-3 py-1.5">
      ...
      <div className="flex gap-1 mb-6 bg-night-900 p-1 rounded-xl w-fit max-w-full overflow-x-auto">
        {[
          { key: "velocity", label: `Velocity (${trips.length})` },
          { key: "recommendations", label: `Prices (${pricingRecs.length})` },
          { key: "competitors", label: `Competitors (${competitors.length})` },
        ].map(tab => (
          <button
            key={tab.key}
            onClick={() => setActiveTab(tab.key as typeof activeTab)}
            className={`px-3 md:px-4 py-1.5 rounded-lg text-sm font-medium transition-all whitespace-nowrap ${
            ...
            <div>
              {trips.length === 0 ? (
                ...
                <div className="space-y-2">
                  {/* Legend */}
                  <div className="flex gap-3 md:gap-4 mb-3 text-xs text-night-500 font-mono flex-wrap">
                    <span><span className="text-red-400">■</span> Critical</span>
                    <span><span className="text-aurora-gold">■</span> Behind</span>
                    <span><span className="text-aurora-blue">■</span> On pace</span>
                    <span><span className="text-aurora-green">■</span> Ahead</span>
                    {company === "tbt" && <span className="sm:ml-auto text-aurora-gold">⚠ = guide cost inflection</span>}
                  </div>

                  {/* Column headers — hidden on mobile */}
                  <div className="hidden lg:flex items-center gap-4 px-4 pb-1 text-xs font-mono text-night-600">
    ...
    >
      ...
      <div className="lg:hidden">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-night-200 text-sm font-medium truncate flex-1">{trip.name}</span>
          <span className="text-night-300 text-sm font-mono flex-shrink-0">
            ${trip.current_price_usd?.toLocaleString()}
          </span>
        </div>
        <div className="flex items-center gap-3 text-xs font-mono text-night-500 mb-2">
          <span>{format(parseISO(trip.departure_date), "MMM d")}</span>
          <span className={trip.days_out > 0 && trip.days_out < 60 ? "text-aurora-gold" : ""}>{trip.days_out > 0 ? `${trip.days_out}d` : "Departed"}</span>
          <span className={statusColors[trip.velocity_status]}>{trip.velocity_status.replace("_", " ")}</span>
          <span className={belowMin ? "text-red-400" : ""}>{trip.bookings_count}/{trip.capacity_max}</span>
        </div>
        {/* Capacity bar */}
      ...
      {expanded && (
        <div className="mt-3 pt-3 border-t border-night-800 ml-0 lg:ml-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6 text-xs font-mono">
          {/* Fill Progress */}
          <div>
      ...
      <div className="flex flex-col gap-3 mb-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3 flex-wrap">
          {competitorRecs.length > 0 && (
            <span className="text-aurora-gold text-xs font-mono">⚠ {competitorRecs.length} price change alert{competitorRecs.length !== 1 ? "s" : ""}</span>
          )}
          <span className="text-night-600 text-xs font-mono hidden sm:inline">Auto-scraped daily at 8am · Click any price to edit</span>
          {regions.length > 0 && (
        ...
        <button onClick={onRunScraper} disabled={runningScraper} className="btn-secondary text-sm px-4 py-2 flex-shrink-0">
        ...
        {competitorNames.map(name => {
          ...
          return (
            <div key={name} className="card">
              {/* Competitor header */}
              <div className="flex flex-col gap-2 mb-4 pb-3 border-b border-night-800 sm:flex-row sm:items-center">
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  <span className="text-aurora-blue text-sm">◈</span>
                  <h3 className="text-night-100 font-medium truncate">{name}</h3>
                  {isStale && <span className="text-xs font-mono text-aurora-gold bg-night-800 px-2 py-0.5 rounded flex-shrink-0">Stale</span>}
                  {scrapeAge !== null && !isStale && (
                    <span className="text-xs text-night-600 font-mono flex-shrink-0">
                      Updated {scrapeAge === 0 ? "today" : `${scrapeAge}d ago`}
                    </span>
                  )}
                  {scrapeAge === null && <span className="text-xs text-night-700 font-mono flex-shrink-0">Never scraped</span>}
                </div>
                {/* Price range summary */}
                {minPrice !== null && (
                  <span className="text-aurora-gold font-mono text-sm flex-shrink-0">
                    {minPrice === maxPrice
                ...
                {minPrice === null && (
                  <span className="text-night-700 text-xs font-mono flex-shrink-0">No prices found — enter manually ↓</span>
                )}
              </div>
                ...
                {products.map(p => {
                  ...
                  return (
                    <div key={p.id} className="bg-night-950 rounded-lg p-3">
                      ...
                      {/* Bottom row */}
                      <div className="flex items-center gap-3 mt-2 pt-2 border-t border-night-800 flex-wrap">
                        ...
                        <span className="text-night-800 text-xs font-mono sm:ml-auto">
